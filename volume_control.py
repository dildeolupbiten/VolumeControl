...
class VolumeControl:
    ...
    widget = VolumeControl(
        ...
        length=5,
        width=4,
        color_gradient={"from": "yellow", "to": "green"}
    )
    button = tk.Button(
        master=root,
        text="Get VolumeControl Value",
        command=lambda: print(widget.value)
    )
    button.pack()
    root.mainloop()
    """
        ...
        for i in range(0, 360, 10):
            self.__master.create_line(
                self.__line_coordinates(
                    r1=self.__distance,
                    r2=(self.__distance + self.__length) + (360 - i) / 10,
                    angle=i
                ),
                width=self.__width,
                tag=f"unit{i}",
